  ...
  return (
    <div className={cn("zev-hero", className)}>
      ...
      {!missing && (
        ...
        <img src={CHARACTER_SRC} alt="Zev character artwork" onError={() => setMissing(true)} />
      )}
...
export function Avatar({ size = 40 }: { size?: number }) {
  ...
  return (
    ...
    >
      {!missing ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={CHARACTER_SRC} alt="Zev" className="h-full w-full object-cover" onError={() => setMissing(true)} />
      ) : (
