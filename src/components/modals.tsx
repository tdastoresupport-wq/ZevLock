...
/** Dialog a11y: Escape dismisses, focus lands inside on open. */
      ...
      >
        ...
        <div className="space-y-3 p-5">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: reduce ? 0 : 0.1, duration: DUR.fast }}
          ...
          <motion.div
            ...
            transition={{ delay: reduce ? 0 : 0.55, duration: DUR.base, ease }}
            className="space-y-1.5 text-center text-[13px] text-slate-300"
          >
            <p className="flex items-center justify-center gap-1.5">
              <Smartphone size={14} className="text-purple-300" /> {device} · Đã gắn
            </p>
            <p className="flex items-center justify-center gap-1.5">
              <ShieldCheck size={14} className="text-emerald-300" /> Phiên đã sẵn sàng
            </p>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: reduce ? 0 : 0.8, duration: DUR.base, ease }}
          >
...
export function LicenseModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-4 sm:items-center" onClick={onClose}>
      <div ref={ref} role="dialog" aria-modal="true" aria-label="Chi tiết bản quyền" className="zev-card w-full max-w-[400px] p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="text-base font-black">Bản quyền</h3>
          <button onClick={onClose} aria-label="Đóng" className="rounded-full border border-white/10 p-1.5 text-slate-400">
            <X size={16} />
          </button>
        </div>
        <div className="mt-3 flex items-center justify-between gap-2 rounded-2xl border border-white/10 bg-black/30 p-3">
          ...
          <button onClick={() => void copy()} aria-label="Chép key" className="zev-btn-ghost shrink-0 !px-2.5 !py-1.5">
            <Copy size={14} />
          </button>
        </div>
        <div className="mt-3 space-y-1.5 text-[13px] text-slate-300">
          <p className="flex items-center justify-between">Gói <span className="zev-badge-premium !px-2.5 !py-1 !text-[11px]">♛ {license.plan}</span></p>
          <p className="flex items-center justify-between">Trạng thái <b className="text-emerald-300">{license.status}</b></p>
          <p className="flex items-center justify-between">Hết hạn <b>{fmtDate(license.expires_at)}</b></p>
          <p className="flex items-center justify-between">Kích hoạt <b>{fmtDate(license.activated_at)}</b></p>
          <p className="flex items-center justify-between">Thiết bị <b>{device.platform} · {device.status}</b></p>
        </div>
        <p className="mt-3 flex items-center justify-center gap-1 text-[11px] text-slate-500">
          <BadgeCheck size={12} /> Đã xác thực bởi Zev Lock
        </p>
      </div>
    </div>
  );
